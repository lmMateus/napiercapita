...
import FilterMT from './components/FilterMT'
  ...
  useEffect(() => {
    const filteredTitle = () => {
      ...
      if (selectedStatus.length > 0) {
        filtered = filtered.filter(title => selectedStatus.includes(title.status_titulo));
      }
      if (priceMin != null) {
        filtered = filtered.filter(title => parseFloat(priceMin) <= parseFloat(title.valor_titulo));
      }
      if (priceMax != null) {
        filtered = filtered.filter(title => parseFloat(priceMax) >= parseFloat(title.valor_titulo));
      }
      if (dateStart != null) {
